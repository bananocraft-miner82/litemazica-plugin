...
public final class FabricWorldAccess implements WorldAccess
{
    ...
    public byte[] captureBlockEntity(int x, int y, int z)
    {
        ...
        {
            NbtIo.writeCompressed(entity.createNbtWithIdentifyingData(), bytes);
    ...
    public void restoreBlockEntity(int x, int y, int z, byte[] blob)
    {
        ...
        {
            NbtCompound nbt = NbtIo.readCompressed(bytes);
            entity.readNbt(nbt);
            entity.markDirty();
        }
        catch (IOException e)
    ...
    public void applyLootTable(int x, int y, int z, String lootTableId, long seed)
    {
        ...
        container.setLootTable(id, seed);
        container.markDirty();
    }
    ...
    /** Builds a trap stack, adding its potion when present (1.20 stores it in the item's NBT). */
    private ItemStack trapStack(DispenserItem item)
    {
        ...
        {
            Potion potion = Registries.POTION.get(Identifier.tryParse(item.potionId()));
    ...
    public void configureSpawner(int x, int y, int z, SpawnerConfig config)
    {
        ...
        entity.readNbt(spawnerNbt(config));
        entity.markDirty();
    }
